    ...
    <section id="Contact">
      ...
      <div className="grid-cols-2 md:grid-cols-3 grid md:ml-16 mb-24">
        <div
          data-aos="fade-right"
          duration="400"
          className="md:col-span-2 col-span-2 md:block flex justify-center md:mr-10 ml-10 mr-10 md:ml-0"
        >
        ...
        <div
          className="md:col-span-1 col-span-2 md:mt-0 mt-5"
          data-aos="fade"
          duration="400"
        >
          <div className="flex justify-center flex-col items-center gap-y-3 indent">
            <h3 className="DMMono text-blue-400 md:text-md sm:text-2xl text-xl">
              Let's get in touch
            </h3>
            <div className="border-2 border-white p-5 rounded-md">
              <form onSubmit={(e) => e.preventDefault()}>
                ...
                {data.fullName.trim().length === 0 && clicked && (
                  <div className="text-red-400">Full name required</div>
                )}
                <InputComponent
                  name="Subject"
                ...
                {data.subject.trim().length === 0 && clicked && (
                  <div className="text-red-400">Subject required</div>
                )}
                <div className="p-0">
                  <label htmlFor="content" className="DMMono text-white">
                    Content
                  </label>
                  <br></br>
                  <textarea
                    ...
                    cols="22"
                    rows="8"
                    className="p-1 min-w-full bg-neutral-700 border-2 border-blue-400 text-white rounded-md placeholder-gray"
                  ></textarea>
                </div>
                {data.content.trim().length === 0 && clicked && (
                  <div className="text-red-400 mt--5">Content required</div>
                )}
                <div>
                  <button
                    type="submit"
                    className={
                      "font-bold DMMono bg-blue-400 text-neutral-700 p-2 mt-3 min-w-full rounded-md " +
                      (data.fullName.trim().length === 0 ||
                      data.subject.trim().length === 0 ||
                      data.content.trim().length === 0
                        ? "bg-blue"
